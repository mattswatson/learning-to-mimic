...
class EyegazeDataset(Dataset):
    def __init__(self, csv_file, image_path_name, class_names, static_heatmap_path=None, heatmaps_path=None, heatmap_temporal_transform=None,
        ...
        # If we're using static heatmaps, only include images we have the heatmaps for
        if self.static_heatmap_path:
            dicoms_with_heatmaps = [p for p in os.listdir(self.static_heatmap_path) if
                                    os.path.isdir(os.path.join(self.static_heatmap_path, p))]
            self.csv_file = self.csv_file[self.csv_file['dicom_id'].isin(dicoms_with_heatmaps)]
